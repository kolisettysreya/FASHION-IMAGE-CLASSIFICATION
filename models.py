...
class CNNClassifier(nn.Module):
    ...
    def visualize_layer_outputs(self, input_image, layers_to_visualize=None, max_features=16):
        # FIX: was missing 'self', now correctly defined as instance method
        self.eval()
        x = input_image.to(next(self.parameters()).device)
...
class VGGClassifier(nn.Module):
    def __init__(self, num_classes):
        ...
        self.model = pt_models.vgg16(weights=pt_models.VGG16_Weights.DEFAULT)
...
class AlexNetClassifier(nn.Module):
    def __init__(self, num_classes):
        ...
        self.model = pt_models.alexnet(weights=pt_models.AlexNet_Weights.DEFAULT)
        for param in self.model.features.parameters():
            param.requires_grad = False
        in_features = self.model.classifier[-1].in_features
        self.model.classifier[-1] = nn.Linear(in_features, num_classes)
...
# FIX: Added missing ResNetClassifier (used in main.py + EncoderClassifierWrapper)
class ResNetClassifier(nn.Module):
    def __init__(self, num_classes, encoded_dim=256):
...
# FIX: Added missing MobileNetClassifier (used in main.py)
class MobileNetClassifier(nn.Module):
    def __init__(self, num_classes):
        super(MobileNetClassifier, self).__init__()
        self.model = pt_models.mobilenet_v2(weights=pt_models.MobileNet_V2_Weights.DEFAULT)
        for param in self.model.features.parameters():
            param.requires_grad = False
        in_features = self.model.classifier[-1].in_features
        self.model.classifier[-1] = nn.Linear(in_features, num_classes)
...
class AutoEncoder(nn.Module):
    def __init__(self, encoded_dim=256):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(256 * 16 * 16, encoded_dim)
        )
        self.decoder = nn.Sequential(
            nn.Linear(encoded_dim, 256 * 16 * 16),
            nn.ReLU(),
...
def get_model_by_name(name, num_classes=None, **kwargs):
    ...
    if name == 'vgg':
        ...
        return VGGClassifier(num_classes)
    if name == 'resnet':
        if num_classes is None:
